...
import { ArrowLeft, Edit, Download, Package, User, Phone, MapPin, Award, Shield, AlertTriangle, Loader2 } from 'lucide-react';
...
export default function ViewProduct() {
  ...
  const toggleLostStatus = async () => {
    try {
      const updatedStatus = !record.is_lost;
      const updatedRecord = await api.updateProduct(id, { is_lost: updatedStatus });
      setRecord(updatedRecord);
      toast.success(`Product marked as ${updatedStatus ? 'LOST' : 'SAFE'}`);
    } catch (err) {
